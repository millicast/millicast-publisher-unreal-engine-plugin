...
uint32_t SumStreamMaxBitrate(const webrtc::VideoCodec& Codec)
{
	return std::accumulate(Codec.simulcastStream, Codec.simulcastStream + Codec.numberOfSimulcastStreams,
		0, [](int c, auto& e) { return e.maxBitrate + c; });
}

int GetNumberOfStreams(const webrtc::VideoCodec& Codec)
{
	return (SumStreamMaxBitrate(Codec) == 0) ? 1 : Codec.numberOfSimulcastStreams;
}

int GetNumActiveStreams(const webrtc::VideoCodec& Codec)
{
	return std::count_if(Codec.simulcastStream, Codec.simulcastStream + GetNumberOfStreams(Codec),
...
{
public:
	AdapterEncodedImageCallback(FSimulcastVideoEncoder* adapter,
		size_t stream_idx)
		: _adapter(adapter), _stream_idx(stream_idx)
	{
	}
	...
	{
		return _adapter->OnEncodedImage(_stream_idx, encoded_image, codec_specific_info);
	}
#endif

private:
	FSimulcastVideoEncoder* const _adapter;
	const size_t _stream_idx;
};

FSimulcastVideoEncoder::FSimulcastVideoEncoder(FSimulcastEncoderFactory& InSimulcastFactory, 
...
FSimulcastVideoEncoder::~FSimulcastVideoEncoder()
{
}

int FSimulcastVideoEncoder::Release()
{
	{
...
int FSimulcastVideoEncoder::InitEncode(const webrtc::VideoCodec* codec_settings, const webrtc::VideoEncoder::Settings& settings)
{
	...
	if (NumActiveStreams == 1)
	{
		...
		const int LastStreamIndex = 0; //  UE::PixelStreaming::Settings::SimulcastParameters.Layers.Num() - 1; // Last stream is highest res.
		FMillicastVideoEncoderFactory* EncoderFactory = SimulcastEncoderFactory.GetEncoderFactory(LastStreamIndex);
	...
	else
	{
		for (int i = 0; i < NumberOfStreams; ++i)
		{
			...
			FMillicastVideoEncoderFactory* EncoderFactory = SimulcastEncoderFactory.GetEncoderFactory(i);
			std::unique_ptr<VideoEncoder> Encoder = EncoderFactory->CreateVideoEncoder(Format);
...
int FSimulcastVideoEncoder::Encode(const webrtc::VideoFrame& input_image, const std::vector<webrtc::VideoFrameType>* frame_types)
{
	if (!IsInitialized())
	{
...
int FSimulcastVideoEncoder::RegisterEncodeCompleteCallback(webrtc::EncodedImageCallback* callback)
{
	EncodedCompleteCallback = callback;
	return WEBRTC_VIDEO_CODEC_OK;
}

void FSimulcastVideoEncoder::SetRates(const RateControlParameters& parameters)
{
	if (!IsInitialized())
	{
...
void FSimulcastVideoEncoder::OnPacketLossRateUpdate(float packet_loss_rate)
{
...
void FSimulcastVideoEncoder::OnRttUpdate(int64_t rtt_ms)
{
	for (StreamInfo& Info : StreamInfos)
	{
...
void FSimulcastVideoEncoder::OnLossNotification(const LossNotification& loss_notification)
{
	for (StreamInfo& Info : StreamInfos)
	{
...
FSimulcastVideoEncoder::EncoderInfo FSimulcastVideoEncoder::GetEncoderInfo() const
{
	if (StreamInfos.size() == 1)
	{
	...
	EncoderInfo.implementation_name = "MillicastPublisherSimulcastEncoderAdapter";
...
bool FSimulcastVideoEncoder::IsInitialized() const
{
	return Initialized;
}

#if WEBRTC_VERSION == 84
webrtc::EncodedImageCallback::Result FSimulcastVideoEncoder::OnEncodedImage(size_t stream_idx, const webrtc::EncodedImage& encoded_image, const webrtc::CodecSpecificInfo* codec_specific_info, const webrtc::RTPFragmentationHeader* fragmentation)
#else
webrtc::EncodedImageCallback::Result FSimulcastVideoEncoder::OnEncodedImage(size_t stream_idx, const webrtc::EncodedImage& encoded_image, const webrtc::CodecSpecificInfo* codec_specific_info)
#endif // WEBRTC_VERSION == 84
{
	webrtc::EncodedImage StreamImage(encoded_image);
	webrtc::CodecSpecificInfo StreamCodecSpecific = *codec_specific_info;
